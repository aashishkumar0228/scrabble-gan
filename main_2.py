...
from src.bigacgan.data_utils import load_prepare_data, train_2, make_gif, load_random_word_list_2
...
from src.bigacgan.multi_digit_dataloader import MultiDigitDataLoader
...
def main():
    ...
    print(read_dir)
    random_words = load_random_word_list_2(read_dir, bucket_size, char_vec)
    ...
    random_bucket_idx = random.randint(2, bucket_size - 1)
    labels = np.array([random.choice(random_words[random_bucket_idx]) for _ in range(num_gen)], np.int32)

    # start training
    train_2(train_dataset, generator, discriminator, recognizer, gan, checkpoint, checkpoint_prefix, generator_optimizer,
